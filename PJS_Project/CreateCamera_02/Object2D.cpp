...
void Object2D::SetPosition(Vector2D _pos , Vector2D _vCamera)
{
    //Vector2D vDrawSize;
...
void Object2D::ScreenToCamera(Vector2D _vCameraPos, Vector2D _vViewPort)
{
    Vector2D vViewPos = m_vPosition;
    Vector2D vDrawSize;
    vDrawSize.x = m_rtInit.xSize / 2.0f;
    vDrawSize.y = m_rtInit.ySize / 2.0f;
    m_rtCollision = { m_vPosition.x - vDrawSize.x, m_vPosition.y - vDrawSize.y, m_rtInit.xSize, m_rtInit.ySize };

    vViewPos.x = m_rtCollision.x0 - _vCameraPos.x;
    vViewPos.y = m_rtCollision.y0 - _vCameraPos.y;
    m_vDrawPos.x = vViewPos.x * (2.0f / _vViewPort.x);
    m_vDrawPos.y = vViewPos.y * (2.0f / _vViewPort.y)*-1.0f;
    m_vDrawSize.x = (m_rtInit.xSize / _vViewPort.x) * 2.0f;
    m_vDrawSize.y = (m_rtInit.ySize / _vViewPort.y) * 2.0f;
}
