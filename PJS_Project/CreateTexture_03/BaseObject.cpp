...
HRESULT BaseObject::CreateVertexBuffer()
{
    ...
    m_pVertexList[0].t = { 0.3f, 0.3f };
    m_pVertexList[1].t = { 0.7f, 0.3f };
    m_pVertexList[2].t = { 0.3f, 0.7f };
    m_pVertexList[3].t = { 0.3f, 0.7f };
    m_pVertexList[4].t = { 0.7f, 0.3f };
    m_pVertexList[5].t = { 0.7f, 0.7f };
...
HRESULT BaseObject::CreateShader(std::wstring _shName)
{
    ...
    hr = D3DCompileFromFile(_shName.c_str(), NULL, NULL, "VS", "vs_5_0", 0, 0, &m_pVSCode, &pErrorCode);            // 쉐이더파일 이름, Nll, Null,함수이름, 버텍스 쉐이더 컴파일러vs_5_0
    ...
    hr = D3DCompileFromFile(_shName.c_str(), NULL, NULL, "PS", "ps_5_0", 0, 0, &m_pPSCode, &pErrorCode);    // 쉐이더파일 이름, Nll, Null,함수이름, 버텍스 쉐이더 컴파일러vs_5_0
    // Blob으로 반환된다. pVSCode,  오류가 생기면 pErrorCode 
    if (FAILED(hr))
    {
        if (pErrorCode)
...
{
    HRESULT hr;
    hr = DirectX::CreateWICTextureFromFile(m_p3dDevice, m_pImmediateContext, _fileName.c_str(), &m_pTexture, &m_pSrvTexture);
    return hr;
}
...
bool BaseObject::Render()
{
    ...
    m_pImmediateContext->PSSetShaderResources(0, 1, &m_pSrvTexture);
