...
bool SceneInGame::Init()
{
    ...
    m_pWallSound = I_Sound.GetPtr(L"IngameSound.mp3");
    m_pGetItem = I_Sound.GetPtr(L"GetItem.mp3");
    m_pMonsterDie = I_Sound.GetPtr(L"Monsteride.mp3");
    
    //m_pWallSound->Play(false);
    SetPlayer();
    SetMap();
    SetTile();
    SetObject();
    ...
    if (!m_pItemBoxList.empty())
    {
        for (auto iter = m_pPlayer->m_pBulletList.begin(); iter != m_pPlayer->m_pBulletList.end(); iter++)
        {
            for (auto jter = m_pItemBoxList.begin(); jter != m_pItemBoxList.end(); jter++)
            {
                ...
                {
                    m_pPlayer->m_bCollsionCheck = false;
                    if ((*jter)->m_iCountItem == 2)
                    {
                        m_pGetItem->PlayEffect();
