...
bool Player::Init()
{
	...
	m_pShoot = I_Sound.GetPtr(L"ShootCannon.mp3");
	m_pBombBoom = I_Sound.GetPtr(L"BulletBomb.wav");
	m_pFireSound = I_Sound.GetPtr(L"fireSound.wav");
	m_pGameOver = I_Sound.GetPtr(L"GameOver.mp3");
	//TankIdle();
	return true;
}
bool Player::Frame()
...
bool Player::Fire()
{
	...
	if (I_Input.GetKey(VK_LBUTTON) == KEY_UP)
	{
		...
		m_iMissileCount--;
		m_bFire = true;
		m_pShoot->PlayEffect(false);
		m_pFireSound->PlayEffect();
	}
	return true;
}
